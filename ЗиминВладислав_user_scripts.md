...
* Для некоторых команд установлена проверка реагирующего, например:
    * Вносить изменения в настройки бота (смена языка/префикса, установленных на сервере) с помощью команды !settings могут только пользователи с правами администратора на сервере
    * Выбирать действия во время какой-либо азартной игры могут только пользователи, инициировавшие игру
...
* Для получения префикса сервера необходимо отправить сообщение "<pr>" (возможно будет изменено) в любой текстовый чат сервера.
* В статусе бота в списке участников сервера всегда находятся команды <pr> и help
...
### [ Сценарий 5 - Использование монет ]
...
### [ Сценарий 8 - Получение информации о пользователе ]
